...
// V2: Pure-Dart TF-IDF + Logistic Regression inference — no native library,
//     fully offline. Model loaded from assets/model_data.json.
//
// Model format (model_data.json):
//   meta.model_type : "LogisticRegression"
//   meta.features   : 50000
//   meta.threshold  : 0.5
//   vocabulary      : { word → feature_index }   (unigrams + bigrams)
//   idf             : [ idf_value … ]            (50 000 values)
//   weights         : [ weight … ]               (50 000 values)
//   intercept       : double
//
...
class _LRClassifier {
  ...
  // Tokenise into unigrams + bigrams (mirrors sklearn ngram_range=(1,2))
  ...
  // Returns sigmoid probability of the text being toxic.
  double score(String text) {
    final tokens = _ngrams(text);
    if (tokens.isEmpty) return 0.0;

    // Raw term counts for known vocabulary entries
    ...
    // TF-IDF: raw_count * IDF
    final tfidf = <int, double>{};
    for (final e in counts.entries) {
      tfidf[e.key] = e.value * idf[e.key];
    }

    // L2 normalise (sklearn TfidfVectorizer default norm='l2')
    var norm = 0.0;
  ...
  bool isToxic(String text) => score(text) >= threshold;
}

// ── Loader (runs heavy JSON parse in a background isolate) ────────────────────
...
class ContentDetectionServiceV2 {
  ...
  Future<void> start() async {
    ...
    debugPrint('SAFECHILD_V2: service started for $_deviceId ✓ '
  ...
  Future<void> stop() async {
    try {
      ...
      debugPrint('SAFECHILD_V2: stream stop error — $e');
    } finally {
  ...
  Future<void> _loadModel() async {
    ...
    try {
      ...
      // Parse in a background isolate — JSON is ~5 MB
      ...
      _classifier = null;
      debugPrint('SAFECHILD_V2: *** MODEL LOAD FAILED ***');
      debugPrint('SAFECHILD_V2: error → $e');
      debugPrint('SAFECHILD_V2: trace → $st');
    }
  }
  ...
  Future<void> _processText(String rawText, String sourceApp) async {
    ...
    debugPrint(
      'SAFECHILD_V2: LR → prob=${prob.toStringAsFixed(3)} '
      'toxic=$toxic  "$cleaned"',
    );

    if (toxic) _handleHarmful(prob, sourceApp, cleaned);
  }
