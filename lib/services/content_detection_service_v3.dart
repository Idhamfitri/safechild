// lib/services/content_detection_service_v3.dart
...
//
// Gemini is only called for the ~uncertain slice, cutting API cost significantly
// while keeping high recall for edge-case Malay/Manglish slang.
//
...
// ---------------------------------------------------------------------------
// Pure-Dart TF-IDF + Logistic Regression classifier (same as V2)
// ---------------------------------------------------------------------------
...
class ContentDetectionServiceV3 {
  ...
  Future<void> start() async {
    ...
    if (isEnabled != true) {
      debugPrint('SAFECHILD_V3: Accessibility not granted. Aborting.');
      return;
    }
    ...
    if (_deviceId == null || _deviceId!.isEmpty) {
      debugPrint('SAFECHILD_V3: device_id not found. Cannot start.');
      return;
    }
    ...
    _accessibilitySubscription =
      ...
      onError: (e) => debugPrint('SAFECHILD_V3: stream error -- $e'),
    );

    debugPrint('SAFECHILD_V3: started for $_deviceId '
  ...
  Future<void> stop() async {
    try {
      ...
      debugPrint('SAFECHILD_V3: stop error -- $e');
    } finally {
      _accessibilitySubscription = null;
      _eventList.clear();
      _classifier = null;
      debugPrint('SAFECHILD_V3: service stopped.');
    }
  }
  ...
  Future<void> _handleQueuedEvent(AccessibilityEvent event) async {
    ...
    debugPrint('SAFECHILD_V3: event from $sourceApp -> "$rawText"');
  ...
  // -------------------------------------------------------------------------
  // Gemini escalation (uncertain zone only)
  // -------------------------------------------------------------------------
...
''';
      ...
      debugPrint('SAFECHILD_V3: Gemini response = $result');

      if (result != null) {
        _parseGeminiResponse(result, sourceApp, rawText: text);
      }
    } catch (e) {
      debugPrint('SAFECHILD_V3: Gemini error -- $e');
    }
  }
  ...
  Future<void> _logIncident({
    ...
    try {
      await _db.collection('incidents').add({
        ...
        'is_alert_send': alert,
        'is_reviewed': false,
        'detector': 'hybrid_v3_$detector', // 'hybrid_v3_local_lr' or 'hybrid_v3_gemini'
      });
      debugPrint('SAFECHILD_V3: Incident logged (via $detector)');
    } catch (e) {
      debugPrint('SAFECHILD_V3: Firestore error -- $e');
    }
  }
}
